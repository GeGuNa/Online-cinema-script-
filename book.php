...
$k_post = mysql_result(mysql_query("SELECT COUNT(*) FROM `bookmark` where `user` = '".$usrname['user_id']."'"),0);
$k_page = k_page($k_post,$user['p_str']);
$page = page($k_page);
$start = $user['p_str']*$page-$user['p_str'];

$info = mysql_query("SELECT * FROM `bookmark` where `user` = '".$usrname['user_id']."' ORDER BY `id` DESC LIMIT $start, $user[p_str]");
...
$filmi = SQL_Fetch("SELECT * FROM films WHERE film_id = ?",array($post['mark']))->fetch();
