...
$k_post = mysql_result(mysql_query("SELECT COUNT(*) FROM `followers` where `user` = '".$usrname['user_id']."'"),0);
$k_page = k_page($k_post,$user['p_str']);
$page = page($k_page);
$start = $user['p_str']*$page-$user['p_str'];

$info = mysql_query("SELECT * FROM `followers` where `user` = '".$usrname['user_id']."' ORDER BY `id` DESC LIMIT $start, $user[p_str]");
...
<?php if ($k_post == 0) { ?>
<div class="widgets-group content-item3 wbg normal-light"> Вы не подписали никого </div>
<? } ?>
