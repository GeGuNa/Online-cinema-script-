...
$k_post = mysql_result(mysql_query("SELECT COUNT(*) FROM `history` where `user` = '".$usrname['user_id']."'"),0);
$k_page = k_page($k_post,$user['p_str']);
$page = page($k_page);
$start = $user['p_str']*$page-$user['p_str'];

$info = mysql_query("SELECT * FROM `history` where `user` = '".$usrname['user_id']."' ORDER BY `id` DESC LIMIT $start, $user[p_str]");
...
$filmi = SQL_Fetch("SELECT * FROM films WHERE film_id = ?",array($post['film']))->fetch();
...
<div class="list-link__text"><?php echo show_text(mb_strimwidth($filmi['film_desc'],0,50,"....")); ?></div>     
</div>  
</a>  
</div>
