...
$quety = SQL_Fetch("SELECT * FROM guest WHERE anke = ? order by `id` desc limit 150",array($usernm['user_id']));


$user['p_str'] = 10;

$k_post = mysql_result(mysql_query("SELECT COUNT(*) FROM `guest` where `anke` = '".$usernm['user_id']."'"),0);
$k_page = k_page($k_post,$user['p_str']);
$page = page($k_page);
$start = $user['p_str']*$page-$user['p_str'];

$info = mysql_query("SELECT * FROM `guest` where `anke` = '".$usernm['user_id']."' ORDER BY `time` DESC LIMIT $start, $user[p_str]");
...
<?php if ($k_post == 0) { ?>
<div class="widgets-group content-item3 wbg normal-light"> Вашу страничку еще не посещали </div>
<? } ?>
...
<div class="list-link__text"><?php echo post_time($post['time']);?></div>     
</div>  
</a>  
...
<div class="replace_widget_wrapper">  <a href="/notifications.php?act=del" class="stnd-link list-link-red c-red js-replace_link "> <span class="js-ico  ico ico_delete "></span> <span class="t js-text">Очистить список гостей</span> </a>   </div>          
</div>    
